...
//       const res = await api.post(`/caregiver/patients/${patient._id}/photo`, formData, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });
//       setForm((f) => ({ ...f, profilePhoto: res.data.profilePhoto }));
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          {isEdit ? (
            <div>
              <label className="text-sm font-bold text-care-text block mb-1">Photo</label>
              <div className="flex items-center gap-4">
                {form.profilePhoto ? (
                  <img
                    src={form.profilePhoto}
                    alt={form.name}
                    className="w-16 h-16 rounded-full object-cover border-2 border-care-border"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-care-bg border-2 border-care-border flex items-center justify-center text-care-muted text-xl font-bold">
                    {form.name?.[0]?.toUpperCase() || "?"}
                  </div>
                )}
                <label className="cursor-pointer text-sm font-bold text-care-primary border-2 border-care-border rounded-xl px-4 py-2 hover:bg-care-bg transition-all">
            ...
            <p className="text-sm text-care-muted -mt-2">
              You can add a photo after saving this patient.
            </p>
          )}
